...
const CitationHandler: React.FC<CitationHandlerProps> = ({
  ...
  useEffect(() => {
    ...
    if (providedCitations && providedCitations.length > 0) {
      ...
      processedText = processedText.replace(
        ...
        (match, id) => {
          ...
          return `<a href="${href}" class="jzy-citation-ref" data-citation-id="${id}" role="button" tabindex="0" aria-label="Citation ${id}">${match}</a>`;
        }
      );
    ...
    if (sourceCitationMatches.length > 0) {      const extractedCitations = sourceCitationMatches.map((match, idx) => {
      ...
      mainContent = mainContent.replace(sourceCitationRegex, () => {
        ...
        return `<sup><a href="${href}" class="jzy-citation-inline" data-citation-id="${citationId}" role="button" tabindex="0" aria-label="Citation ${citationId}">${citationId}</a></sup>`;
      }).trim();
    }
      ...
      processedText = processedText.replace(
        ...
        (match: string, id: string) => {
          ...
          if (citationIds.has(id)) {
            ...
            const href = `#citation-${id}`;
            return `<a href="${href}" class="jzy-citation-ref jzy-superscript" data-citation-id="${id}" role="button" tabindex="0" aria-label="Citation ${id}">${id}</a>`;
          }
      ...
      (match: string, text: string, url: string) => {
        ...
        if (text === 'FOCUS Overview' || text === 'FOCUS Metadata Details') {
          ...
          if (citationId) {
            ...
            return `<a href="#citation-${citationId}" class="jzy-citation-ref jzy-educational-link" data-citation-id="${citationId}"><span class="jzy-reference-icon">📖</span> ${text}</a>`;
          }
          
          // If no citation found, create a simple link that will stay in the app
          return `<a href="#" class="jzy-external-link jzy-educational-link"><span class="jzy-reference-icon">📖</span> ${text}</a>`;
        }
        ...
        if (url === '#' || !url) {
          const citationId = extractedCitations.find(c => c.title?.trim() === text.trim())?.id;
            ...
            if (isEducationalLink) {
              return `<a href="${href}" class="jzy-citation-ref jzy-educational-link" data-citation-id="${citationId}" role="button" tabindex="0" aria-label="Citation for ${text}"><span class="jzy-reference-icon">📖</span> ${text}</a>`;
            }
            
            return `<a href="${href}" class="jzy-citation-ref" data-citation-id="${citationId}" role="button" tabindex="0" aria-label="Citation for ${text}">${text}</a>`;
          }
        }
        ...
        if (matchedCitation) {
          ...
          return `<a href="#citation-${matchedCitation.id}" class="jzy-citation-ref" data-citation-id="${matchedCitation.id}" role="button" tabindex="0" aria-label="Citation for ${text}">${text}</a>`;
        }
        
        // For all other URLs, keep as just text without link to avoid external navigation
        return `<span class="jzy-external-link-disabled">${text}</span>`;
      }
    );
  ...
  if (citations.length > 0) {
    ...
    displayContent = displayContent.replace(
      ...
      (_match, id) => {
        const href = `#citation-${id}`;
        return `<sup><a href="${href}" class="jzy-citation-inline" data-citation-id="${id}" role="button" tabindex="0" aria-label="Citation ${id}">${id}</a></sup>`;
      }
    );
  ...
  return (
    <div className="jzy-citation-handler">
      <div dangerouslySetInnerHTML={{ __html: displayContent }} />      {citations.length > 0 ? (        <div className="jzy-citation-card-list" aria-label="Citations and References" role="region" style={{
          marginTop: '30px', 
          ...
          <div className="jzy-citation-cards-container" style={{
            display: 'flex',
            ...
            {citations.map((citation) => (              <div
              ...
              >                <div className="jzy-citation-pill-number" style={{
                  display: 'inline-flex',
                  alignItems: 'center',
                ...
                }}>{citation.id}</div>                <div className="jzy-citation-card-title" style={{
                  fontWeight: 600, 
                  marginBottom: '6px',
                  fontSize: '1.05em',
                  color: '#222'
                ...
                {citation.section && (
                  <div className="jzy-citation-card-section" style={{
                    marginBottom: '4px', 
                    color: '#0078d4', 
                    fontSize: '0.95em',
                    display: 'flex',
                ...
                )}                <div className="jzy-citation-card-docname" style={{
                  color: '#444', 
                  fontSize: '0.93em', 
                  marginTop: '4px', 
                  fontStyle: 'italic'
                ...
                {citation.file_name && (
                  <div className="jzy-citation-card-author" style={{
                    fontSize: '0.85em', 
                    color: '#666',
                    marginTop: '3px',
                    display: 'flex',
